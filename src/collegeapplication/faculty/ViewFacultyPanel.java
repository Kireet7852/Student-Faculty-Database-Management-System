...
public class ViewFacultyPanel extends JPanel {
	...
	private ViewFacultyPanel(Faculty f)
	{
		...
		assignsubjectbutton.setVisible(true);
		...
		deletebutton.setBounds(590, 141, 140, 35);
	...
	public ViewFacultyPanel(Faculty f, AdminMain am,JComponent lastpanel)
	{
		...
		JLabel lblJoinedDate = new JLabel("Account created Date    ");			//Joined Date
		...
		deletebutton.addActionListener(new ActionListener()				//for deletion of the table
		{
			...
			public void actionPerformed(ActionEvent arg0) {
					...
					// as.setVisible(true);
					int result=0;
					int popup=JOptionPane.showConfirmDialog(null,"Do you want to delete "+f.getFacultyName()+"?","Delete Faculty",JOptionPane.INFORMATION_MESSAGE);
					if(popup == JOptionPane.YES_OPTION){
						result = new FacultyData().deleteFaculty(f);
						am.viewfacultypanel.setVisible(false);
						if(lastpanel.getName().equals("Faculty Panel"))
						{
							if(am.facultypanel.viewbutton.getText().equals("Photo View"))
